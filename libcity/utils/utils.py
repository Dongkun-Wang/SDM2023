...
def get_executor(config, model, data_feature, teacher=None):
    """
    ...
    try:
        return getattr(importlib.import_module('libcity.executor'),
                       config['executor'])(config, model, data_feature,teacher)
...
def get_logger(config, name=None):
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_filename = '{}-{}-{}-{}.log'.format(config['model'], config['dataset'], get_local_time(),config['exp_id'])
