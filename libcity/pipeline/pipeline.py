...
def run_model(task=None, model_name=None, dataset_name=None, config_file=None,
              saved_model=True, train=True, other_args=None, teacher_model=None):
    """
    Args:
        task(str): task name
    ...
    # 加载执行器
    if config['model']=='PP':
        hyper_parameter = 'dim_'+str(config['embed_dim'])+'_layer_'+str(config['num_layers'])+'_head_'+str(config['num_heads'])+'_dropout_'+str(config['dropout'])
    elif config['model']=='PPD':
        hyper_parameter = 'teacher_'+config['teacher']+'_distill_'+config['distill']+'_dropout_'+str(config['dropout'])
    else:
        hyper_parameter = 'dropout_'+str(config['dropout'])
        
    model_cache_file = './libcity/cache/{}_{}/{}_id_{}/model_cache/{}_{}.m'.format(
        config['model'],config['dataset'],hyper_parameter,exp_id, model_name, dataset_name)
    model = get_model(config, data_feature)
    teacher = 'None'
    if teacher_model:
        teacher_config = ConfigParser(task, teacher_model, dataset_name,
                          config_file, saved_model, train, other_args)
        teacher = get_model(teacher_config, data_feature)
    executor = get_executor(config, model, data_feature, teacher)
    # 训练
