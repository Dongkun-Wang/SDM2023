...
class Transformer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.embedding_layer = torch.nn.Linear(self.num_nodes,self.embed_dim)
        self.activation = nn.GELU()
        self.dropout = nn.Dropout(0.1)
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=self.num_heads, dropout=0.3)
        self.backbone_model = torch.nn.TransformerEncoder(encoder_layer, self.num_layers)
        self.output_layer = torch.nn.Linear(self.embed_dim,self.num_nodes)
