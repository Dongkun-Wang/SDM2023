...
class TrafficStateExecutor(AbstractExecutor):
    def __init__(self, config, model, data_feature, teacher):
        ...
        if self.config['model']=='PP':
            hyper_parameter = 'dim_'+str(self.config['embed_dim'])+'_layer_'+str(self.config['num_layers'])+'_head_'+str(self.config['num_heads'])+'_dropout_'+str(self.config['dropout'])
        elif self.config['model']=='PPD':
            hyper_parameter = 'teacher_'+str(self.config['teacher'])+'_distill_'+str(self.config['distill'])+'_dropout_'+str(self.config['dropout'])
        else:
            hyper_parameter = 'dropout_'+str(self.config['dropout'])

        self.cache_dir = './libcity/cache/{}_{}/{}_id_{}/model_cache'.format(self.config['model'],self.config['dataset'],hyper_parameter,self.exp_id)
        self.evaluate_res_dir = './libcity/cache/{}_{}/{}_id_{}/evaluate_cache'.format(self.config['model'],self.config['dataset'],hyper_parameter,self.exp_id)
        self.summary_writer_dir = './libcity/cache/{}_{}/{}_id_{}/'.format(self.config['model'],self.config['dataset'],hyper_parameter,self.exp_id)
        ...
        self.patience = self.config.get('patience', 30)
        ...
        if not teacher is None and teacher != 'None':
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, loss_func=None):
        ...
        for batch in train_dataloader:
            ...
            batch.to_tensor(self.device)
            if self.config['model']=='PPD':
                teacher_model_name = type(self.teacher_model).__name__  
                teacher_model_path =  "./libcity/teacher_model/{}_{}.m".format(teacher_model_name,self.config['dataset'],teacher_model_name,self.config['dataset'])  
                
                '''
                teacher_model_path ="./libcity/cache/TGCN_SZ_TAXI/dropout_0.3_id_6056/model_cache/TGCN_SZ_TAXI_epoch299.tar"
                checkpoint = torch.load(teacher_model_path, map_location='cpu')
                self.teacher_model.load_state_dict(checkpoint['model_state_dict'])
                '''

                #teacher_model_path = "./libcity/cache/TGCN_SZ_TAXI/dropout_0.3_id_77932/model_cache/TGCN_SZ_TAXI.m"
                model_state, optimizer_state = torch.load(teacher_model_path)
                self.teacher_model.load_state_dict(model_state)
                with torch.no_grad():
                    teacher_output = self.teacher_model.predict(batch)
                loss = loss_func(batch,teacher_output)
            else:
